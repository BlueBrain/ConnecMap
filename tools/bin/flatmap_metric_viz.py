...
In this version it is possible to query a 3D ball inside the volume of interest and the
metric analysis for this ball only. In this case, all considered distances in the source space are
distances  between the center of the ball and a voxel of the ball.
See the main() function help documentation.
...
def compute_distance_map(
    flatmap: 'voxcell.VoxelData', voxel_sample_size: int = 2000,
) -> Dict[int, List[float]]:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--ball_center',
        type=str,
        help='Indices i,j,k of the voxel used as the center of a ball sample.',
        default=None,
    )
    parser.add_argument(
        '--ball_radius',
        type=int,
        help='Radius of the ball used as a sample when taking distances between pairs of voxels.'
        ' Defaults to 10',
        default=12,
    )
    ...
    ball_sample = None
    if args.ball_center is not None:
        volume_mask = flatmap.raw[..., 0] >= 0
        ball_center = split_str(args.ball_center, int)
        ball_sample = get_ball(
            volume_mask, ball_center, args.ball_radius
        )

    if args.voxel_sample_size is None and ball_sample is None:
        raise AssertionError(
            'Either voxel_sample_size must be set or both'
            ' ball_center and ball_radius must be set.'
        )

    if args.voxel_sample_size is not None:
        distance_map = compute_distance_map(
            flatmap, voxel_sample_size=args.voxel_sample_size
        )
    else:
        distance_map = compute_local_distance_map(flatmap, ball_sample, ball_center)
